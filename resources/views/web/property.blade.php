    ...
    <section class="main_property">
        <div class="main_property_header py-5 bg-light">
            <div class="container">
                ...
                <p class="mb-0">{{ $property->getPtCategory() }} - {{ $property->getPtType() }} - {{ $property->neighborhood }}</p>
            </div>
        </div>
                    ...
                    <div class="col-12 col-lg-8">
                            ...
                            <ol class="carousel-indicators">
                                ...
                                @if ($property->images()->get()->count())
                                    @foreach ($property->images()->get() as $image)
                                        <li data-target="#carouselProperty" data-slide-to="{{ $loop->iteration }}" class="{{ ($loop->iteration == 1 ? 'active' : '') }}"></li>
                                    @endforeach
                                @endif
                                ...
                                @if ($property->images()->get()->count())
                                    @foreach ($property->images()->get() as $image)
                                        <div class="carousel-item {{ ($loop->iteration == 1 ? 'active' : '') }}">
                                            <a href="{{ $image->getUrlCroppedAttribute(1600, 900) }}" data-toggle="lightbox"
                                            data-gallery="property-gallery" data-type="image">
                                                <img src="{{ $image->getUrlCroppedAttribute(1600, 900) }}"
                                                    class="d-block w-100"
                                                    alt="...">
                                            </a>
                                        </div>
                        ...
                        <div class="main_property_price pt-4 text-muted">
                            ...
                            @else {{-- Caso seja acessado por experiência --}}
                                ...
                                <p class="main_property_price_big">R$ {{ $property->sale_price }} <br>
                                ...
                                @elseif ($property->rent == true && !empty($property->rent_price))
                                    <p class="main_property_price_big">Valor do Aluguel: R$ {{ $property->rent_price }}/mês</p>
                                @else
                                    <p class="main_property_price_big">R$ {{ $property->rent_price }}Entre em contato com a nossa equipe comercial!</p>
                                @endif    
                            @endif
                        </div>
                    ...
                    <div class="col-12 col-lg-4">
                        ...
                        <div class="main_property_contact">
                            ...
                            <form action="{{ route('web.sendEmail') }}" method="post" autocomplete="off">
                                ...
                                <div class="form-group">
                                    <label for="cell">Seu telefone:</label>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="message" id="message" cols="30" rows="5" class="form-control">Quero ter mais informações sobre esse imóvel. {{ $property->getPtCategory() }}, {{ $property->getPtType() }}, {{ $property->neighborhood }}, {{ $property->city }}! (#{{ $property->id }})</textarea>
                                </div>
