    ...
    <div class="dash_content_app_box">
        <div class="nav">
            ...
            <form class="app_form" action="{{ route('admin.users.update', ['user' => $user->id]) }}" method="post" enctype="multipart/form-data">
                ...
                {{-- Id informado para validação no form request --}}
                ...
                <div class="nav_tabs_content">
                    <div id="data">
                        <div class="label_gc">
                            ...
                            <label class="label">
                                <input type="checkbox" name="lessor" {{ (old('lessor') == true || old('lessor') == 'on' ? 'checked' : ($user->lessor == true ? 'checked' : '')) }}><span>Locatário</span>
                            </label>

                            <label class="label">
                                <input type="checkbox" name="lessee" {{ (old('lessee') == true || old('lessee') == 'on' ? 'checked' : ($user->lessee == true ? 'checked' : '')) }}><span>Locador</span>
                            </label>
                        </div>
                        ...
                        <div class="label_g2">
                            <label class="label">
                                ...
                                <input type="text" class="mask-doc-sec" name="document_secondary" placeholder="RG do Cliente"
                                       value="{{ old('document_secondary') ?? $user->document_secondary }}"/>
                            </label>

                            <label class="label">
                                <span class="legend">Órgão Expedidor:</span>
                                <input type="text" class="mask-doc-sec-comp" name="document_secondary_complement" placeholder="Expedição"
                            ...
                            <label class="label">
                                ...
                                <select name="civil_status">
                                    <optgroup label="Cônjuge Obrigatório">
                                        <option value="married" {{ (old('civil_status') == 'married' ? 'selected' : ($user->civil_status == 'married' ? 'selected' : '')) }}>Casado</option>
                                        <option value="separated" {{ (old('civil_status') == 'separated' ? 'selected' : ($user->civil_status == 'separated' ? 'selected' : '')) }}>Separado</option>
                                    </optgroup>
                                    <optgroup label="Cônjuge não Obrigatório">
                                        <option value="single" {{ (old('civil_status') == 'single' ? 'selected' : ($user->civil_status == 'single' ? 'selected' : '')) }}>Solteiro</option>
                                        <option value="divorced" {{ (old('civil_status') == 'divorced' ? 'selected' : ($user->civil_status == 'divorced' ? 'selected' : '')) }}>Divorciado</option>
                                        <option value="widower" {{ (old('civil_status') == 'widower' ? 'selected' : ($user->civil_status == 'widower' ? 'selected' : '')) }}>Viúvo</option>
                                    </optgroup>
                                </select>
                            </label>
                            ...
                            <div class="app_collapse_content d-none content_spouse">
                                ...
                                <label class="label">
                                    ...
                                    <select name="type_of_communion" class="select2">
                                        <option value="Comunhão Universal de Bens" {{ (old('type_of_communion') == 'Comunhão Universal de Bens' ? 'selected' : ($user->type_of_communion == 'Comunhão Universal de Bens' ? 'selected' : '')) }}>Comunhão Universal de Bens</option>
                                        <option value="Comunhão Parcial de Bens" {{ (old('type_of_communion') == 'Comunhão Parcial de Bens' ? 'selected' : ($user->type_of_communion == 'Comunhão Parcial de Bens' ? 'selected' : '')) }}>Comunhão Parcial de Bens</option>
                                        <option value="Separação Total de Bens" {{ (old('type_of_communion') == 'Separação Total de Bens' ? 'selected' : ($user->type_of_communion == 'Separação Total de Bens' ? 'selected' : '')) }}>Separação Total de Bens</option>
                                        <option value="Participação Final de Aquestos" {{ (old('type_of_communion') == 'Participação Final de Aquestos' ? 'selected' : ($user->type_of_communion == 'Participação Final de Aquestos' ? 'selected' : '')) }}>Participação Final de Aquestos
                                        </option>
                                    </select>
                                </label>
                                ...
                                <div class="label_g2">
                                    <label class="label">
                                        ...
                                        <select name="spouse_genre">
                                            <option value="male" {{ (old('spouse_genre') == 'male' ? 'selected' : ($user->spouse_genre == 'male' ? 'selected' : '')) }}>Masculino</option>
                                            <option value="female" {{ (old('spouse_genre') == 'female' ? 'selected' : ($user->spouse_genre == 'female' ? 'selected' : '')) }}>Feminino</option>
                                            <option value="other" {{ (old('spouse_genre') == 'other' ? 'selected' : ($user->spouse_genre == 'other' ? 'selected' : '')) }}>Outros</option>
                                        </select>
                                    </label>
                                ...
                                <div class="label_g2">
                                    <label class="label">
                                        <span class="legend">RG:</span>
                                        <input type="text" class="mask-doc-sec" name="spouse_document_secondary"
                                               placeholder="RG do Cliente" value="{{ old('spouse_document_secondary') ?? $user->spouse_document_secondary }}"/>
                                    </label>

                                    <label class="label">
                                        <span class="legend">Órgão Expedidor:</span>
                                        <input type="text" class="mask-doc-sec-comp" name="spouse_document_secondary_complement"
                        ...
                        <div class="app_collapse mt-2">
                            ...
                            <div class="app_collapse_content">
                                ...
                                <div class="companies_list">
                                    {{-- Caso exista algum relacionamento --}}
                            ...
                            <div class="app_collapse_content">
                                <div id="realties">
                                        <div class="realty_list">
                                            @if (count($user->properties()->get()) > 0)
                                                @foreach ($user->properties()->orderBy('id','DESC')->get() as $property)
                                                    ...
                                                    <div class="realty_list_item mb-1">
                                                        ...
                                                        <div class="realty_list_item_content">
                                                            {{-- Usando as traduções --}}
                                                            <h4>#{{ $property->id }} {{ $pt_category }} - {{ $pt_type }}</h4>
                                                        ...
                                                        <div class="realty_list_item_actions">
                                                            ...
                                                            <div>
                                                                @if ($property->sale == true && !empty($property->sale_price))
                                                                <a href="{{ route('web.buyProperty', ['slug' => $property->slug]) }}" class="btn btn-blue icon-eye" target="_blank">Visualizar Imóvel (Compra)</a>   
                                                                @endif
                                                                @if ($property->rent == true && !empty($property->rent_price))
                                                                <a href="{{ route('web.rentProperty', ['slug' => $property->slug]) }}" class="btn btn-blue icon-eye" target="_blank">Visualizar Imóvel (Locação)</a>
                                                                @endif
                                    ...
                                    <div class="realty_list">
                                        @if (count($user->contractsAsAcquirer()->get()) > 0)
                                            @foreach ($user->contractsAsAcquirer()->get() as $contract)
                                                {{-- Se Conseguir obter algum contrato, salve em property --}}
                                                @if ($property = $contract->propertyRelation()->first())
                                                    {{-- Traduções --}}
                                                    @php
                                                        if(!empty($property->category)){
                                                            switch ($property->category) {
                    ...
                    <div id="management" class="d-none">
                        <div class="label_gc">
                            ...
                            <label class="label">
                                <input type="checkbox" name="admin" {{ (old('admin') == true || old('admin') == 'on' ? 'checked' : ($user->admin == true ? 'checked' : '')) }}><span>Administrativo</span>
                            </label>

                            <label class="label">
                                <input type="checkbox" name="client" {{ (old('client') == true || old('client') == 'on' ? 'checked' : ($user->client == true ? 'checked' : '')) }}><span>Cliente</span>
                            </label>
                        </div>
                    </div>
                </div>
