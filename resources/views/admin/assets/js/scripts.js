$(function () {
    ...
    $(".mask-doc-sec").mask('00.000.000-0', {reverse: true});
    $(".mask-doc-sec-comp").mask('AAA/AA', {reverse: true});
    ...
    // BLOQUEIA CAMPOS DO FORM SPOUSE

    function normalizeSpouse(){
        // Se o tipo do seletor for diferente d undefined...
        if(typeof($('select[name="civil_status"]')) !== 'undefined'){
            // Se civil status igual a casado ou separado...
            if($('select[name="civil_status"]').val() === 'married' ||
                $('select[name="civil_status"]').val() === 'separated'){
                // Os inputs e selects de content_spouse terão a propriedade disabled como falso
                $('.content_spouse input, .content_spouse select').prop('disabled', false);
            } else {
                // Os inputs e selects de content_spouse terão a propriedade disabled como true
    ...
    // Verifica a troca do seletor de civil status
    $('select[name="civil_status"]').change(function(){
        // Dispara a função novamente
        normalizeSpouse();
    }) 

    // ENABLE INPUTS SALE E RENT
