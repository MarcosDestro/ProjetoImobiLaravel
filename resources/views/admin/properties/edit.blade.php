        ...
        <div class="nav">
            ...
            <form action="{{ route('admin.properties.update', ['property' => $property->id]) }}" method="post" class="app_form" enctype="multipart/form-data">
                ...
                <div class="nav_tabs_content">
                    <div id="data">
                        <div class="label_gc">
                            ...
                            <label class="label">
                                <input type="checkbox" name="sale" {{ (old('sale') == 'on' || old('sale') == 'on' ? 'checked' : ($property->sale == true ? 'checked' : '')) }}><span>Venda</span>
                            </label>

                            <label class="label">
                                <input type="checkbox" name="rent" {{ (old('rent') == 'on' || old('rent') == 'on' ? 'checked' : ($property->rent == true ? 'checked' : '')) }}><span>Locação</span>
                            </label>
                        </div>
                    ...
                    <div id="structure" class="d-none">
                        ...
                        <div class="label_g5">
                            <div>
                                <label class="label">
                                    <input type="checkbox" name="air_conditioning" {{ (old('air_conditioning') == 'on' || old('air_conditioning') == 'on' ? 'checked' : ($property->air_conditioning == true ? 'checked' : '')) }}><span>Ar Condicionado</span>
                                </label>
                            </div>
                            <div>
                                <label class="label">
                                    <input type="checkbox" name="bar" {{ (old('bar') == 'on' || old('bar') == 'on' ? 'checked' : ($property->bar == true ? 'checked' : '')) }}><span>Bar</span>
                                </label>
                            </div>
                            <div>
                                <label class="label">
                                    <input type="checkbox" name="library" {{ (old('library') == 'on' || old('library') == 'on' ? 'checked' : ($property->library == true ? 'checked' : '')) }}><span>Biblioteca</span>
                                </label>
                            </div>

                            <div>
                                <label class="label">
                                    <input type="checkbox" name="barbecue_grill" {{ (old('barbecue_grill') == 'on' || old('barbecue_grill') == 'on' ? 'checked' : ($property->barbecue_grill == true ? 'checked' : '')) }}><span>Churrasqueira</span>
                                </label>
                            </div>

                            <div>
                                <label class="label">
                                    <input type="checkbox" name="american_kitchen" {{ (old('american_kitchen') == 'on' || old('american_kitchen') == 'on' ? 'checked' : ($property->american_kitchen == true ? 'checked' : '')) }}><span>Cozinha Americana</span>
                                </label>
                            </div>

                            <div>
                                <label class="label">
                                    <input type="checkbox" name="fitted_kitchen" {{ (old('fitted_kitchen') == 'on' || old('fitted_kitchen') == 'on' ? 'checked' : ($property->fitted_kitchen == true ? 'checked' : '')) }}><span>Cozinha Planejada</span>
                                </label>
                            </div>
                            <div>
                                <label class="label">
                                    <input type="checkbox" name="pantry" {{ (old('pantry') == 'on' || old('pantry') == 'on' ? 'checked' : ($property->pantry == true ? 'checked' : '')) }}><span>Despensa</span>
                                </label>
                            </div>

                            <div>
                                <label class="label">
                                    <input type="checkbox" name="edicule" {{ (old('edicule') == 'on' || old('edicule') == 'on' ? 'checked' : ($property->edicule == true ? 'checked' : '')) }}><span>Edícula</span>
                                </label>
                            </div>

                            <div>
                                <label class="label">
                                    <input type="checkbox" name="office" {{ (old('office') == 'on' || old('office') == 'on' ? 'checked' : ($property->office == true ? 'checked' : '')) }}><span>Escritório</span>
                                </label>
                            </div>
                            <div>
                                <label class="label">
                                    <input type="checkbox" name="bathtub" {{ (old('bathtub') == 'on' || old('bathtub') == 'on' ? 'checked' : ($property->bathtub == true ? 'checked' : '')) }}><span>Banheira</span>
                                </label>
                            </div>
                            <div>
                                <label class="label">
                                    <input type="checkbox" name="fireplace" {{ (old('fireplace') == 'on' || old('fireplace') == 'on' ? 'checked' : ($property->fireplace == true ? 'checked' : '')) }}><span>Lareira</span>
                                </label>
                            </div>
                            <div>
                                <label class="label">
                                    <input type="checkbox" name="lavatory" {{ (old('lavatory') == 'on' || old('lavatory') == 'on' ? 'checked' : ($property->lavatory == true ? 'checked' : '')) }}><span>Lavabo</span>
                                </label>
                            </div>
                            <div>
                                <label class="label">
                                    <input type="checkbox" name="furnished" {{ (old('furnished') == 'on' || old('furnished') == 'on' ? 'checked' : ($property->furnished == true ? 'checked' : '')) }}><span>Mobiliado</span>
                                </label>
                            </div>
                            <div>
                                <label class="label">
                                    <input type="checkbox" name="pool" {{ (old('pool') == 'on' || old('pool') == 'on' ? 'checked' : ($property->pool == true ? 'checked' : '')) }}><span>Piscina</span>
                                </label>
                            </div>
                            <div>
                                <label class="label">
                                    <input type="checkbox" name="steam_room" {{ (old('steam_room') == 'on' || old('steam_room') == 'on' ? 'checked' : ($property->steam_room == true ? 'checked' : '')) }}><span>Sauna</span>
                                </label>
                            </div>
                            <div>
                                <label class="label">
                                    <input type="checkbox" name="view_of_the_sea" {{ (old('view_of_the_sea') == 'on' || old('view_of_the_sea') == 'on' ? 'checked' : ($property->view_of_the_sea == true ? 'checked' : '')) }}><span>Vista para o Mar</span>
                                </label>
                            </div>
                        </div>
                            ...
                            @foreach ($property->images()->get() as $image)
                                <div class="property_image_item">
                                    ...
                                    <div class="property_image_actions">
                                        <a href="javascript:void(0)" class="btn icon-check icon-notext image-set-cover {{ ($image->cover == true ? 'btn-green' : '') }}"
                                            data-action="{{ route('admin.properties.imageSetCover', ['image' => $image->id]) }}"></a>
                                        <a href="javascript:void(0)" class="btn btn-red icon-times icon-notext image-remove"
                ...
                <div class="text-right mt-2">
                    <button class="btn btn-large btn-green icon-check-square-o">Alterar Imóvel</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    $(function () {
        ...
        /** Funções de atribuição cover */
        $('.image-set-cover').click(function(event){

            event.preventDefault();
            var button = $(this);
            // URL de envio, dados, ação de retorno, tipo retorno esperado
            $.post(button.data('action'), {}, function(response){
                if(response.success === true){
                    // Encontre todos os botões verde e retire a classe
                    $('.property_image').find('a.btn-green').removeClass('btn-green');
                    // Adicione a classe verde ao botão clicado
        ...
        $('.image-remove').click(function(event){
            ...
            $.ajax({
                ...
                success: function(response){
                    if(response.success === true){
                        // Seleciona o elemento pai com a classe abaixo e dê um fadeout
                        button.closest('.property_image_item').fadeOut(function(){
                            // Depois remova esse elemento do html
                            $(this).remove();
                        });
                    }
                }
