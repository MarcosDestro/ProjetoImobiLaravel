            ...
            <div class="nav_tabs_content">
                <div id="parts">
                    <form action="{{ route('admin.contracts.update', ['contract' => $contract->id]) }}" method="post" class="app_form">
                        ...
                        {{-- Inputs hidden para manipulação do ajax --}}
                        ...
                        <div class="label_gc">
                            ...
                            <label class="label">
                                ...
                                {{ (old('purpose') == 'sale' ? 'checked' : ($contract->purpose == 'sale' ? 'checked' : '')) }}><span>Venda</span>
                            </label>

                            <label class="label">
                                <input type="radio" name="purpose" value="rent"
                                {{ (old('purpose') == 'rent' ? 'checked' : ($contract->purpose == 'rent' ? 'checked' : '')) }}><span>Locação</span>
                            </label>
                        </div>
                                    ...
                                    <label class="label">
                                        ...
                                        data-action="{{ route('admin.contracts.getDataOwner') }}">
                                            ...
                                            @foreach ($lessors as $lessor)
                                                <option value="{{ $lessor->id }}" {{ (old('owner') == $lessor->id ? 'selected' : ($lessor->id == $contract->owner ? 'selected' : '')) }}>{{ $lessor->name }} ({{ $lessor->document }})</option>
                                            @endforeach
                                        </select>
                                    </label>
                                    ...
                                    <label class="label">
                                        ...
                                        data-action="{{ route('admin.contracts.getDataAcquirer') }}">
                                            ...
                                            @foreach ($lessees as $lessee)
                                                <option value="{{ $lessee->id }}" {{ (old('acquirer') == $lessee->id ? 'selected' : ($lessee->id == $contract->acquirer ? 'selected' : '')) }}>{{ $lessee->name }} ({{ $lessee->document }})</option>
                                            @endforeach
                                        </select>
                                    </label>
                        ...
                        <div class="text-right mt-2">
                            <button class="btn btn-large btn-green icon-check-square-o">Alterar Contrato</button>
                        </div>
                    </form>
                </div>
    ...
    <script>
        $(function(){
            ...
            function setFieldOwner(response){
                /** Cônjuge */
                $('select[name="owner_spouse"]').html(''); // Zera o campo
                    if(response.spouse){ // Se tiver resposta de spouse...
                        ...
                        $('select[name="owner_spouse"]').append($('<option>',{
                            ...
                            // Se spouse persist for diferente de 0, marque ele
                    ...
                    /** Empresas */
                    $('select[name="owner_company"]').html(''); // Zera o campo
                    // Verifica se houve uma contave, pois temos um coletivo
                    if(response.companies != null && response.companies.length){ // Se tiver itens em compansies...
                        ...
                        $.each(response.companies, function(key, value){
                            $('select[name="owner_company"]').append($('<option>',{
                                value: value.id, // Seta o id da empresa
                                text: value.alias_name + ' (' + value.document_company + ')',
                                // Tem que existir um valor e deve ser igual ao id do loop
                    ...
                    /** Imóveis */
                    $('select[name="property"]').html(''); // Zera o campo
                    // Verifica se houve uma contave, pois temos um coletivo
                    if(response.properties != null && response.properties.length){ // Se tiver itens em compansies...
            ...
            function setFieldAcquirer(response){
                /** Cônjuge */
                $('select[name="acquirer_spouse"]').html(''); // Zera o campo
                    if(response.spouse){ // Se tiver resposta de spouse...
                    ...
                    /** Empresas */
                    $('select[name="acquirer_company"]').html(''); // Zera o campo
                    // Verifica se houve uma contave, pois temos um coletivo
                    if(response.companies != null && response.companies.length){ // Se tiver itens em compansies...
            ...
            $('select[name="owner"]').change(function(){
                var owner = $(this);
                $.post(owner.data('action'), {id: owner.val()}, function(response){
                    setFieldOwner(response);
                }, 'json')
            });

            /** Owner Spouse */
            // Se o select owner tiver valor, dispara o ajax
            ...
            $('select[name="acquirer"]').change(function(){
                var acquirer = $(this);
                $.post(acquirer.data('action'), {id: acquirer.val()}, function(response){
            ...
            $('select[name="property"]').change(function(){
                var property = $(this);
                $.post(property.data('action'), {id: property.val()}, function(response){
            ...
            //     $.post(property.data('action'), {id: $('input[name="property_persist"]').val()}, function(response){
            //         setFieldProperty(response);
            //     }, 'json')
            // }

            
            // var checks = document.querySelectorAll('input.checkgroup');
            // checks.forEach((elem)=>{
            //     alert(checks.value);
            // })
